...
> available: he loses the behaviour he asked for, and finds out after the release.
>
...
> second silent revert: a comment at the flag or call site stating that the behaviour is
...
4. **Re-publish via the same route `ship` used for this PR** — Route A
   (`npm run publish`) for a monorepo contribution, Route B's mirror flow if that is
   the PR's topology. Do not force Route A onto a Route B extension. Expect and handle
   Route A's `pull-contributions` dance per `ship` (CHANGELOG rebuilds from
