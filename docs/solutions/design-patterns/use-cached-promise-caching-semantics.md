---
title: "useCachedPromise: four caching behaviours that every gate passes"
date: 2026-09-17
...
# useCachedPromise: four caching behaviours that every gate passes
...
(behaviour 2), a transient failure becomes completely silent unless you raise one yourself.
...
Behaviour 1 also survives single-item manual testing, which is the testing most likely to happen.
...
dependency's runtime behaviour, read the dependency** — `node_modules/@raycast/utils/dist/module.js`
...
- **The moment the same component renders for many rows.** That is when behaviour 1 becomes live,
  and it is the cheapest to check: does the args array contain the value that varies?
- **The moment you write a second cache alongside the hook's.** Behaviour 3 applies, and so does
...
- **Whenever an error branch decides what renders.** Behaviour 2 plus the one-surface rule.
...
- `/Users/messina/Developer/GitHub/chrismessina/raycast-extension-workflows/docs/solutions/workflow-issues/answer-a-blocking-review-with-a-measurement.md`
  — behaviour 1 was flagged by a
