...
This is the same instrument as [read the version off the running artifact](/Users/messina/Developer/dotfiles/claude/docs/solutions/workflow-issues/read-the-version-off-the-running-artifact-not-repo-master.md), pointed at a remedy instead of a bug.
...
Removing it took the status, its icon case, the regex, the counter guard, and four tests that only ever exercised the author's own regex. A test suite that green-lights a deleted premise was measuring nothing. Compare [retire the code, keep the model](/Users/messina/Developer/dotfiles/claude/docs/solutions/workflow-issues/grading-a-reverse-engineered-model-against-the-vendor-release.md) §5.
...
- [Answer a blocking review finding with a measurement](/Users/messina/Developer/GitHub/chrismessina/raycast-extension-workflows/docs/solutions/workflow-issues/answer-a-blocking-review-with-a-measurement.md) — the parent; this extends its §1 to remedies that were already built, and amends its §6.
- [Self-review does not catch diagnostic errors](/Users/messina/Developer/GitHub/chrismessina/raycast-extension-workflows/docs/solutions/workflow-issues/self-review-does-not-catch-diagnostic-errors.md) — the same root cause with the wrong theory held by the author; this is the third instance, where the theory is the reviewer's and adopted by the author. Consolidate if a fourth appears.
- [Read the version off the running artifact](/Users/messina/Developer/dotfiles/claude/docs/solutions/workflow-issues/read-the-version-off-the-running-artifact-not-repo-master.md) — ground truth over source-as-read.
- [Grading a reverse-engineered model against the vendor release](/Users/messina/Developer/dotfiles/claude/docs/solutions/workflow-issues/grading-a-reverse-engineered-model-against-the-vendor-release.md) — §5, retire rather than refine.
