...
It carries `name` (slug-normalised: lowercase, no `--` or `..`, max 64 chars), `description`,
...
- **`captureMemorySnapshot`** gained a much fuller doc comment (behaviour and signature unchanged) —
...
hook type surface is byte-identical. Everything below is CLI behaviour plus the `AI.Model` enum.
...
2. **"Some common shortcuts have also changed" broke the linter.** The app rebound five constants;
   ...
   --fix` will therefore silently *change behaviour* on those five rather than just renaming a
   literal.
